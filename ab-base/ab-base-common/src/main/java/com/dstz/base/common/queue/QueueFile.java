...
/**
 * Based on file queue, thread safe
 *
 */
public class QueueFile implements AutoCloseable {
	...
	public QueueFile(File file) throws IOException {
		if (!file.exists()) {
			...
			this.fileLock = (FileLock) Objects.requireNonNull(arr[1], "File lock not obtained");
		} else {
			raf = open(file);
			this.fileLock = Objects.requireNonNull(raf.getChannel().tryLock(), "File lock not obtained");
		}
		readHeader();
	}
	...
	private static Object[] initialize(File file) throws IOException {
		...
		FileLock fileLock = Objects.requireNonNull(raf.getChannel().tryLock(), "File lock not obtained");
